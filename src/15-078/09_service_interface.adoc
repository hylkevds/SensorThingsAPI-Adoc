...
If one of the two operands of a built-in filter operator is a property of data type __Any__ and the other is a literal or a property not of data type __Any__ nor String, the service MUST include only those items of type __Any__ whose values are of the same JSON data type as the literal/other property and fulfil the query.</p>

If one of the two operands is a property of data type String and the other is a literal or property not of data type string, the values of all items are cast to string before the operator is applied and all items that fulfil the query MUST be included in the result.

If both operands of a built-in filter operator are properties of type __Any__ the service MUST cast both operands to type String.
...
. if number is a valid data type for the parameter, call this version of the function, ignoring all records that do not have the type number.
. otherwise, if string is a valid data type for the parameter, cast the value to string
. else respond with 400 Bad Request.
